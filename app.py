...
# 2. JS İLE OTO-YENİLEME (Kütüphane gerektirmez)
...
def get_static_elements():
    # Arka planda süzülen elmaslar (küçük, kare şeklinde)
    ...
    # YENİ: Süzülen kameralar ve elmaslar AYRI OLARAK oluşturuldu
    camera_floats = "".join([f'<div class="camera-float" style="left:{random.randint(0, 95)}%; animation-duration:{random.randint(15, 25)}s; animation-delay:{random.uniform(0, 10)}s;">📷</div>' for i in range(10)])
    diamond_floats = "".join([f'<div class="diamond-float" style="left:{random.randint(0, 95)}%; animation-duration:{random.randint(10, 20)}s; animation-delay:{random.uniform(0, 10)}s;">💎</div>' for i in range(10)])
    
    return diamonds, camera_floats, diamond_floats

diamonds, camera_floats, diamond_floats = get_static_elements()

# 5. CSS + HTML (Hata Giderildi: Çift süslü parantez)
st.markdown(f"""
<style>
...
html, body, [data-testid="stAppViewContainer"] {{
    ...
    color: #FFD700;
    overflow-x: hidden;
    background-image: repeating-linear-gradient(-45deg, #000 0px, #000 60px, rgba(255, 215, 0, 0.1) 61px, rgba(255, 215, 0, 0.1) 63px);
...
/* Süzülen kameralar ve elmaslar */
.camera-float, .diamond_float {{ position: absolute; font-size: 1.1rem; opacity: 0; animation: floatUp linear infinite; filter: brightness(0.7) sepia(1); }}
...
[data-testid="stImage"] {{ transition: all 0.6s ease; border: 1px solid rgba(255,215,0,0.25); background: rgba(255,215,0,0.03); margin-bottom: 150px !important; }}
[data-testid="column"]:nth-child(2) [data-testid="stImage"] {{ margin-top: 150px; }}
[data-testid="stImage"]:hover {{ transform: scale(1.03); border: 1px solid #FFD700; box-shadow: 0 0 60px rgba(255,215,0,0.35); }}
.visitor-badge {{ position: fixed; bottom: 30px; left: 30px; font-size: 0.6rem; color: #FFD700; letter-spacing: 4px; opacity: 0.8; z-index: 20; display: flex; align-items: center; gap: 8px; }}
#MainMenu, footer, header {{visibility: hidden;}}
</style>
<div id="intro-layer"><div class="intro-text">Utku Çimen</div></div>
<div class="bg-overlay">{diamonds}{camera_floats}{diamond_floats}</div>
