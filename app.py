...
/* SADECE GEREKSİZLERİ GİZLE (HEADER ARTIK VAR!) */
#MainMenu, footer, [data-testid="stSidebarNav"] {{
    visibility: hidden;
}}

/* HEADER ŞEFFAF (BUTON KALSIN) */
header {{
...
# 4. SIDEBAR
with st.sidebar:
    st.markdown("<h2 style='color:#00ffff; font-weight:100;'>EDITOR</h2>", unsafe_allow_html=True)
    ...
    st.caption("Sol üstteki '>' ile paneli aç/kapat")
...
<div class="header-container">
    ...
    <div class="sub-title">
        {archive_year} / PERSONAL ARCHIVE / {len(uploaded_images) if uploaded_images else 0} WORKS
    </div>
</div>
""", unsafe_allow_html=True)
...
if uploaded_images:
    ...
    st.info("← Soldan foto yükle")

# 7. FOOTER
st.markdown("<br><br><br>", unsafe_allow_html=True)
st.markdown(f"""
<p style='text-align: center; color: #333; font-size: 14px; letter-spacing: 4px;'>
PORTFOLIO BY {user_name.upper()}
</p>
""", unsafe_allow_html=True)
