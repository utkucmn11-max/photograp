...
# 4. ARKA PLAN ÖĞELERİ (Yönler ve Belirginlik Ayarlandı)
@st.cache_data
def get_static_elements():
    # Elmaslar: YUKARIDAN AŞAĞIYA (Glow/Parlama eklendi)
    diamond_floats = "".join([f'<div class="diamond-float" style="left:{random.randint(0, 95)}%; animation-duration:{random.randint(8, 15)}s; animation-delay:{random.uniform(0, 5)}s;">💎</div>' for i in range(15)])
    
    # Kameralar: AŞAĞIDAN YUKARIYA
    camera_floats = "".join([f'<div class="camera-float" style="left:{random.randint(0, 95)}%; animation-duration:{random.randint(12, 20)}s; animation-delay:{random.uniform(0, 5)}s;">📷</div>' for i in range(12)])
...
/* ELMASLAR: Yukarıdan Aşağıya ve Daha Belirgin */
.diamond-float {{ 
    position: absolute; 
    font-size: 1.5rem; 
...
.visitor-badge {{ position: fixed; bottom: 30px; left: 30px; font-size: 0.6rem; color: #FFD700; letter-spacing: 4px; opacity: 0.8; z-index: 20; display: flex; align-items: center; gap: 10px; }}
